...
import php.codeGolf.training.learn.lesson.LessonProcessor
import php.codeGolf.training.learn.lesson.XmlLesson
import php.codeGolf.training.learn.lesson.kimpl.KLesson
import php.codeGolf.training.learn.lesson.kimpl.LessonContext
import php.codeGolf.training.learn.lesson.kimpl.LessonExecutor
import php.codeGolf.training.learn.lesson.listeners.NextLessonListener
import php.codeGolf.training.learn.lesson.listeners.StatisticLessonListener
import php.codeGolf.training.project.ProjectUtils
import php.codeGolf.training.ui.LearnToolWindowFactory
import php.codeGolf.training.ui.UiManager
import php.codeGolf.training.util.findLanguageByID
    ...
    private fun openLesson(projectWhereToStartLesson: Project, lesson: Lesson) {
        ...
        try {
            val langSupport = LangManager.getInstance().getLangSupport()
                    ?: throw Exception("Language for learning plugin is not defined")
    ...
    private fun getFileInLearnProject(lesson: Lesson): VirtualFile {
        val function = object : Computable<VirtualFile> {
            ...
            override fun compute(): VirtualFile {
                ...
                val roots = getRoots(learnProject)
                for (file in roots) {
            ...
            private fun getRoots(learnProject: Project): Array<out VirtualFile> {
                var roots = ProjectRootManager.getInstance(learnProject).contentSourceRoots
                if (roots.isEmpty()) {
                    roots = ProjectRootManager.getInstance(learnProject).contentRoots
                }
                return roots
            }
        }
